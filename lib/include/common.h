...
// Initializes an array with the specified capacity, the length will be zero.
...
// Checks if the given byte array has enough space to hold length.
// Returns true if no space is available, false otherwise.
int QTZ_ByteArray_CantHoldLength(QTZ_ByteArray *self, size_t length);
